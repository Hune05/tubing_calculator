...
class _SettingsScreenState extends State<SettingsScreen> {
          ...
          children: [
            TwoColumnRow(
              left: _buildDropdownWithHelper(
                ...
                helperText: "※ 제조사 선택에 따라 우측 가이드가 변합니다.",
              ),
              right: _buildDropdownWithHelper(
                label: "장비 타입 선택",
                value: _benderType,
                ...
                helperText: "※ 수동/전동에 맞춰 우측 가이드가 변합니다.",
              ),
            ),
            ...
            if (_isElectric) ...[
              TwoColumnRow(
                left: _buildNumpadInput(
                  ...
                  _rController,
                  key: 'radius',
                  helperText: "※ 장착된 벤드 슈(Die)의 R값",
                ),
                right: _buildNumpadInput(
                  "클램프 물림 길이 [mm]",
                  _minStraightController,
                  key: 'minStraight',
                  helperText: "※ 튜브 클램프 확보 최소 구간",
                ),
              ),
              const SizedBox(height: 12),
              TwoColumnRow(
                left: _buildNumpadInput(
                  ...
                  _gainController,
                  key: 'gain',
                  helperText: "※ 90도 기준 튜브가 늘어나는 양",
                ),
                right: _buildNumpadInput(
                  "스프링백 보상 [°]",
                  _springbackController,
                  helperText: "※ 각도 복원값 (보통 1~3° 입력)",
                ),
              ),
              const SizedBox(height: 12),
              TwoColumnRow(
  ...
  // ===========================================================================
  // 🚀 오른쪽 (RIGHT): 일체형 스마트 가이드 패널 + 꿀단지 참조표
  // ===========================================================================
  List<Widget> _buildRightGuideGroup() {
    return [
      ...
      _buildHoneyJarReferenceCard(), // 🚀 대망의 꿀단지 3종 세트 추가!
    ];
  }
  ...
  // 🚀 [신규 추가] 3대 꿀단지 공통 참고 도표 (U벤딩, NPT나사산, 인치환산)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            "나사 조립 시 피팅이 포트 안으로 먹어 들어가는 길이입니다. 총 기장 산출 시 이 값을 빼주어야 정확합니다. (Hand-tight + Wrench makeup 기준)",
            style: TextStyle(
              fontSize: 11,
              color: Colors.black54,
              fontWeight: FontWeight.bold,
                  ...
                  ? Row(
                      ...
                      children: [
                        Expanded(
                          flex: 5,
                          child: Column(
                            children: _buildLeftInputSettingsGroup(),
                          ),
                        ),
                        const SizedBox(width: 24),
                        Expanded(
                          flex: 5,
                          child: Column(children: _buildRightGuideGroup()),
                        ),
                      ],
                    )
                  : Column(
...
// 🚀 입력창 배경색 처리 (수동 MAN 모드일 때 연주황색으로 강조!)
