...
class _MarkingPageState extends State<MarkingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: isStandalone
          ? AppBar(
              title: const AppHeaderTitle('마킹 가이드'),
          ...
          : null,
      // 🚀 [수정] BendDataManager를 구독하지 않고 그냥 한 번 읽고 끝냈던 문제를
      // 고쳤다. 이제 ListenableBuilder로 감싸서, 다른 화면(예: 계산기 입력
      // 화면)에서 벤드 리스트가 바뀌면 이 페이지가 열려 있는 동안에도 자동으로
      // 다시 계산되어 반영된다.
      body: SafeArea(
        child: ListenableBuilder(
          listenable: BendDataManager(),
          builder: (context, child) {
            final dataManager = BendDataManager();
            final bendList = dataManager.bendList;

            // 🚀 [수정] 반경 값을 takeUp90이 아니라 실제 radius 필드에서 읽는다.
            // 예전엔 dataManager.takeUp90을 반경으로 잘못 대입해서, 이 화면의
            // 모든 마킹 포인트/총 절단 길이 계산이 틀린 값(테이크업 값) 기준으로
            // 나오고 있었다.
            ...
            // 🚀 [버그 수정] springback(스프링백 보상)/benderOffset(장비 원점
            // 오프셋)이 설정 화면에 저장만 되고 실제 연산에는 전달되지 않던
            // 문제를 고쳤다.
                  ...
                  child: displayMarks.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            int incrementalMark = item['incremental_mark']
                                .round();
