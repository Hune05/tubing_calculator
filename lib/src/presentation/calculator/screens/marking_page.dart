...
class _MarkingPageState extends State<MarkingPage> {
  ...
  void initState() {
    ...
    // 💡 변경점: 텍스트가 바뀔 때 DataManager에도 값을 즉각 동기화
  ...
  Future<void> _refreshSettings() async {
    ...
    if (mounted) {
      setState(() {
        // 💡 초기 로드 시 DataManager의 기존 설정값을 가져옴
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < bendList.length; i++) {
      ...
      // 누적 마킹 계산
      double markPoint =
          cumulativeLength + startFitting - totalGain - currentTakeUp;
      cumulativeMarks.add(markPoint);
      totalGain += currentGain;

      Map<String, dynamic> bendWithMarking = Map<String, dynamic>.from(
        bendList[i],
      );
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(
                      child: _buildSimpleSwitch(
                        "시작 피팅 (+${fittingDepth.round()})",
            ...
            child: bendList.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      int prevMark = index == 0
                          ? 0
                          : cumulativeMarks[index - 1].round();
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    Icon(
                                      _getDirectionIcon(bend['rotation']!),
                                      size: 16,
                                      color: makitaTeal,
                                    ),
                                    const SizedBox(width: 4),
                                    Text(
                                      "${bend['angle']?.round()}° / ${_getDirectionText(bend['rotation']!)}",
                                      style: TextStyle(
                                        color: makitaTeal,
                                        fontSize: 14,
                                        fontWeight: FontWeight.bold,
  ...
  void _handleSave(double totalCut, List<Map<String, dynamic>> saveList) {
    final finalSaveData = List<Map<String, dynamic>>.from(saveList);
