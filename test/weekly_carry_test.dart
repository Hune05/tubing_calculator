...
void main() {
  test('unfinished last-week schedule is carried over; older is late; summary first', () {
    ...
    final logs = [
      {
        ...
        'schedules': [
          {'id': 's1', 'title': '지난주끝', 'isCompleted': false,
    ...
    expect(d.sections.first.heading.startsWith('이번주 요약'), true);
    final all = d.sections.expand((s) => s.lines).join('\n');
    // 이번주가 월요일이면 today가 그 주에 속하므로 항상 성립
    expect(all.contains('지난주끝 (지난주 이월)'), true);
